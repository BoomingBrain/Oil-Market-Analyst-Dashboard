...
function parseOilPriceRSS(xmlText) {
  const items = [];
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (headline) {
        ...
        if (pubDate) {
          ...
          if (diffMins < 60) {
            ...
            if (diffHours < 24) {
              ...
              timeStr = date.toLocaleDateString(void 0, { month: "short", day: "numeric" });
            }
          }
        }
      } catch (e) {
      ...
      items.push({
        ...
        time: timeStr,
        headline,
        category: category.includes("Crude") || category.includes("Oil") ? "Upstream" : category.includes("Gas") ? "Midstream" : "Geopolitical",
        source: "OilPrice.com",
...
var finbertPipeline = null;
var isFinbertLoading = false;
var finbertFailed = false;
function getFallbackSentiment(text) {
  ...
  return parseFloat(Math.max(-1, Math.min(1, score + randomFactor)).toFixed(4));
}
async function getFinbertSentiment(text) {
  if (finbertFailed) {
...
app.get("/api/spreads", async (req, res) => {
  try {
    ...
    const rbMap = /* @__PURE__ */ new Map();
    rbData.timestamps.forEach((ts, i) => {
      if (rbData.closes[i] !== null && rbData.closes[i] !== void 0) {
        rbMap.set(formatDate(ts), rbData.closes[i]);
      }
    });
    const hoMap = /* @__PURE__ */ new Map();
    hoData.timestamps.forEach((ts, i) => {
      if (hoData.closes[i] !== null && hoData.closes[i] !== void 0) {
    ...
    const fallbackSpreads = [
      { timeIndex: "04/17", gasolineCrack: 21.4, distillateCrack: 28.5, crack321: 23.77 },
      { timeIndex: "04/24", gasolineCrack: 22.8, distillateCrack: 29.1, crack321: 24.9 },
      { timeIndex: "05/01", gasolineCrack: 24.1, distillateCrack: 30.5, crack321: 26.23 },
      { timeIndex: "05/08", gasolineCrack: 23.5, distillateCrack: 31, crack321: 26 },
      { timeIndex: "05/15", gasolineCrack: 25.2, distillateCrack: 32.4, crack321: 27.6 },
      { timeIndex: "05/22", gasolineCrack: 26.8, distillateCrack: 33.5, crack321: 29.03 }
    ];
...
app.get("/api/eia-data", async (req, res) => {
  ...
  try {
    ...
    const sprMap = /* @__PURE__ */ new Map();
    sprData.forEach((item) => {
      if (item.period && item.value !== void 0) {
        sprMap.set(item.period, Number(item.value) / 1e3);
      }
    });
    const prodMap = /* @__PURE__ */ new Map();
    prodData.forEach((item) => {
      if (item.period && item.value !== void 0) {
        prodMap.set(item.period, Number(item.value) / 1e3);
      }
    });
    const combined = [];
    crudeData.forEach((item) => {
      const date = item.period;
      if (!date) return;
      const crudeVal = Number(item.value) / 1e3;
...
app.get("/api/rigs-data", async (req, res) => {
  ...
  try {
    ...
    const [oilData, gasData, totalData] = await Promise.all([
      fetchEia("E_ERTRRO_XR0_NUS_C"),
      fetchEia("E_ERTRRG_XR0_NUS_C"),
      fetchEia("E_ERTRR0_XR0_NUS_C")
    ]);
    if (!oilData.length || !gasData.length || !totalData.length) {
      throw new Error("Received empty datasets from EIA API");
    }
    const gasMap = /* @__PURE__ */ new Map();
    gasData.forEach((item) => {
      if (item.period && item.value !== void 0) {
        gasMap.set(item.period, Number(item.value));
      }
    });
    const totalMap = /* @__PURE__ */ new Map();
    totalData.forEach((item) => {
      if (item.period && item.value !== void 0) {
        totalMap.set(item.period, Number(item.value));
      }
    });
    const combined = [];
    oilData.forEach((item) => {
      const date = item.period;
      if (!date) return;
      const oilVal = Number(item.value);
      const gasVal = gasMap.get(date) || 0;
      const totalVal = totalMap.get(date) || oilVal + gasVal;
      combined.push({
        date,
        oilRigs: oilVal,
        gasRigs: gasVal,
    ...
    const statsList = combined.map((curr, idx) => {
      const prev = idx > 0 ? combined[idx - 1] : null;
      const oilChange = prev ? curr.oilRigs - prev.oilRigs : 0;
      const gasChange = prev ? curr.gasRigs - prev.gasRigs : 0;
      const dateParts = curr.date.split("-");
      const weekStarting = dateParts.length === 2 ? `${dateParts[1]}/${dateParts[0].slice(-2)}` : curr.date;
      return {
        weekStarting,
        oilRigs: curr.oilRigs,
        oilChange,
        gasRigs: curr.gasRigs,
        gasChange,
        totalRigs: curr.totalRigs
      };
    });
    const result = statsList.slice(-6);
    res.json({
    ...
    console.warn("Failed to fetch EIA rig counts, falling back to simulated data:", err.message);
    res.json({
      data: fallbackData,
      isSimulated: true
    });
...
app.post("/api/analyze-news", async (req, res) => {
  ...
  if (!ai) {
    ...
    if (lowercase.includes("cut") || lowercase.includes("draw") || lowercase.includes("disrupt") || lowercase.includes("strike") || lowercase.includes("leak") || lowercase.includes("attack") || lowercase.includes("closure")) {
      ...
      impactTimeline = [
        { time: "1 Hour", priceDelta: 1.8, description: "Immediate market speculative repricing as algorithmic trading triggers on alert." },
        { time: "24 Hours", priceDelta: 3.2, description: "Physical pricing adjusts. Spot physical premiums rise (Midland vs. WTI widens)." },
        { time: "3 Days", priceDelta: 4.5, description: "Peak impact as logistics bottlenecks and vessel schedules adjust to avoid chokepoint." },
        { time: "1 Week", priceDelta: 3.8, description: "Secondary storage release dampens absolute climb; price stabilizes slightly lower than peak." },
        { time: "3 Weeks", priceDelta: 2.1, description: "New shipping paths established or refinery restarts begin, easing physical bottlenecks." }
      ];
    } else if (lowercase.includes("build") || lowercase.includes("increase") || lowercase.includes("supply") || lowercase.includes("oversupply") || lowercase.includes("refill") || lowercase.includes("surplus") || lowercase.includes("slowdown") || lowercase.includes("drop")) {
      sentiment = "Bearish";
      confidence = 85;
      ...
      impactTimeline = [
        { time: "1 Hour", priceDelta: -1.2, description: "Algorithmic selling drives high-frequency re-evaluation of near-month contracts." },
        { time: "24 Hours", priceDelta: -2.3, description: "Physical crude market discounts amplify as storage hubs begin filling near capacity." },
        { time: "3 Days", priceDelta: -3.5, description: "Peak inventory pressure creates localized supply bottlenecks, forcing steeper spot discounts." },
        { time: "1 Week", priceDelta: -2.8, description: "Producers adjust pumping rates or start floating storage, slowing physical price decline." },
        { time: "3 Weeks", priceDelta: -1.7, description: "Low price stimulates refinery demand, triggering corrective physical purchasing and stabilization." }
      ];
    } else {
      ...
      impactTimeline = [
        { time: "1 Hour", priceDelta: 0.1, description: "Minor baseline fluctuations on typical futures trading volumes." },
    ...
    res.json({
      ...
      impactTimeline,
      targetStop: {
        target: sentiment === "Bullish" ? 4.5 : sentiment === "Bearish" ? -3.5 : 0.5,
        stop: sentiment === "Bullish" ? -1.5 : sentiment === "Bearish" ? 1.2 : -0.3
      },
      finbertScore
    });
    return;
        ...
        responseSchema: {
          ...
          properties: {
            ...
            impactTimeline: {
              ...
              items: {
                ...
                properties: {
                  ...
                  priceDelta: { type: import_genai.Type.NUMBER },
            ...
            targetStop: {
              ...
              properties: {
                target: { type: import_genai.Type.NUMBER },
                stop: { type: import_genai.Type.NUMBER }
              }
            }
          }
        }
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            impactRating: { type: import_genai.Type.STRING }
          }
        }
      }
    });
